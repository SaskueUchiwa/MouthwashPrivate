...
export class Perspective extends BaseRoom {
    ...
    constructor(
        ...
        for (const [ , player ] of parentRoom.players) {
            const newPlayer = new PlayerData(this, player.clientId);
    ...
    async broadcast(
        ...
        const includedConnections = includedConnection ? [ includedConnection ] : undefined;

        const notCanceledOutgoingGameData = await this.getNotCanceledOutgoing(messages, MessageDirection.Clientbound, undefined);
        const notCanceledOutgoingPayloads = await this.getNotCanceledOutgoing(payloads, MessageDirection.Clientbound, undefined);

        this.broadcastMessages(messages, payloads, includedConnections, undefined, reliable);
        ...
        if (notCanceledOutgoingGameData.length > 0 && notCanceledOutgoingPayloads.length > 0) {
            ...
            if (notCanceledRoomGameData.length > 0 || notCanceledRoomPayloads.length > 0)
                this.parentRoom.broadcastMessages(notCanceledRoomGameData, notCanceledRoomPayloads, includedConnections, undefined, reliable);
            ...
            for (let i = 0; i < this.parentRoom.activePerspectives.length; i++) {
                ...
                if (notCanceledPerspectiveGameData.length > 0 || notCanceledPerspectivePayloads.length > 0) {
                    otherPerspective.broadcastMessages(notCanceledPerspectiveGameData, notCanceledPerspectivePayloads, includedConnections, undefined, reliable);
                }
            }
        }
    }
        ...
        if (restoreState) {
            ...
            for (const [ , player ] of this.parentRoom.players) {
                ...
                messages.push(
                    new RpcMessage(
                        playerControl.netId,
                        new SetNameMessage(playerInfo.name)
                    )
                );
                ...
                messages.push(
                    new RpcMessage(
                        playerControl.netId,
                        new SetColorMessage(playerInfo.color)
                    )
                );
                ...
                messages.push(
                    new RpcMessage(
                        playerControl.netId,
                        new SetHatMessage(playerInfo.hat)
                    )
                );
                ...
                messages.push(
                    new RpcMessage(
                        playerControl.netId,
                        new SetPetMessage(playerInfo.pet)
                    )
                );
                ...
                messages.push(
                    new RpcMessage(
                        playerControl.netId,
                        new SetSkinMessage(playerInfo.skin)
                    )
                );
                ...
                for (let i = 0; i < playerInfo.taskStates.length; i++) {
                    const taskState = playerInfo.taskStates[i];
