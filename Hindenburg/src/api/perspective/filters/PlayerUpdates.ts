import { GameData, PlayerControl, PlayerInfo, RpcMessageTag } from "@skeldjs/core";
import {
    DataMessage,
    RpcMessage
...
export class PlayerUpdatesPerspectiveFilter extends PerspectiveFilter {
    ...
    */
    ...
    protected _applyAllowedInfo(allowedBitfield: number, srcPlayerInfo: PlayerInfo, destPlayerInfo: PlayerInfo) {
        if ((allowedBitfield & PlayerUpdatesFilterFlag.Name) > 0) destPlayerInfo.setName(srcPlayerInfo.name);
        if ((allowedBitfield & PlayerUpdatesFilterFlag.Color) > 0) destPlayerInfo.setColor(srcPlayerInfo.color);
        if ((allowedBitfield & PlayerUpdatesFilterFlag.Hat) > 0) destPlayerInfo.setHat(srcPlayerInfo.hat);
        if ((allowedBitfield & PlayerUpdatesFilterFlag.Skin) > 0) destPlayerInfo.setSkin(srcPlayerInfo.skin);
        if ((allowedBitfield & PlayerUpdatesFilterFlag.Pet) > 0) destPlayerInfo.setPet(srcPlayerInfo.pet);
        ...
        if (perspective.gameData && perspective.parentRoom.gameData) {
            if (direction === MessageFilterDirection.Incoming) {
                ...
                if (perspective.gameData.Serialize(writer, false)) {
                    ...
                    perspective.stream.push(dataMessage);
                }
                ...
                if (perspective.parentRoom.gameData.Serialize(writer, false)) {
                    ...
                    perspective.messageNonce.add(dataMessage);
                    perspective.parentRoom.stream.push(dataMessage);
                }
