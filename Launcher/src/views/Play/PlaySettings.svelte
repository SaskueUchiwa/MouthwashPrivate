    ...
    <div class="bg-[#1a0428] w-1/4 rounded-xl shadow-lg px-6 p-4">
        <div class="flex flex-col h-full gap-2">
            ...
            <div class="flex rounded-lg">
                ...
                {:else}
                    <input class="outline-none flex-1 rounded-r-lg bg-[#1a0428] w-64 text-xs border-none font-inherit text-inherit" placeholder="Location" readonly value={$gameInstalledPathState}>
                {/if}
            </div>
            ...
            <button class="rounded-lg bg-[#27063e] px-4 py-1 self-end mt-4 hover:bg-[#1C072B] hover:text-[#bba1ce] filter"
                on:click={close}
            >
                Close
            </button>
